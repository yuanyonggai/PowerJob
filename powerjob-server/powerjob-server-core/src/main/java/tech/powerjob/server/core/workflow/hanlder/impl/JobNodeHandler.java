...
public class JobNodeHandler implements TaskNodeHandler {
    ...
    public void createTaskInstance(PEWorkflowDAG.Node node, PEWorkflowDAG dag, WorkflowInstanceInfoDO wfInstanceInfo) {
        ...
        Long instanceId = SpringUtils.getBean(InstanceService.class).create(node.getJobId(), wfInstanceInfo.getAppId(), node.getNodeParams(), wfInstanceInfo.getWfContext(), null, wfInstanceInfo.getWfInstanceId(), System.currentTimeMillis()).getInstanceId();
